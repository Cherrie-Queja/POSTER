...
# from visualize.attention_visuialize import *
...
parser.add_argument('-b', '--batch-size', default=150, type=int, metavar='N')
...
def main():
    ...
    # create model
    model = pyramid_trans_expr(img_size=224, num_classes=7, type='large') #手动修改num_classes
    model.cuda()

    # model = BaseLine(img_size=224, num_classes=8).cuda()
    ...
    if args.evaluate is not None:
        ...
        validate(val_loader, model, CE_criterion, lsce_criterion, args)
        return
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            if (count == 0):
                ...
                preds = torch.cat((preds, pred.T), dim=0)
            count += 1
            output = pred
